...
if (! function_exists('justg_share')) {
    /**
     * Backward-compatible share helper for legacy child themes.
     *
     * Mirrors the output of Velocity_Addons_Shortcode::velocity_sharepost().
     *
     * @param array $atts Optional display attributes.
     * @return string
     */
    function justg_share($atts = array())
    {
        $atts = shortcode_atts(
            array(
        ...
        $allowed_platforms = array_map('trim', explode(',', $atts['platforms']));
        $all_links         = array(
            'facebook' => array(
        ...
        echo '<p class="fw-bold mb-1">' . esc_html($atts['title']) . '</p>';
